...
export class FootballBetElementComponent implements OnInit, OnDestroy {
  ...
  setMatch(): void{
    if(!this.match){
      this.matchService.getMatche(this.match.id, this.competitionID).subscribe(
          x=>{
            this.match=x;
            this.setTeams();
          })
  ...
  postBetting() :void{
    ...
    this.sub.push(
        this.betService.postNewBet(bet).subscribe(_ => {location.reload()}, error => {
          if(error.status===401){
  ...
  defineBet(): Bet{
    ...
    bet.match_info = {
      competition: parseInt(this.competitionID? (this.competitionID==='0'? '2002':this.competitionID) : '2002'),
