      ...
      if (!this.loginForm.invalid){
        this.loginSub = this.authService
          ...
          next => {
            ...
            {
              this.router.navigate(['/'], {queryParams: {connection: 'success'}}).then(
                  () => {
                      location.reload();
                  }
              );
              ...
              this.messageService.add({severity: 'error', summary: 'Authentification', detail: next.errors[0] });
              this.showButtonAndNoSpinner = !this.showButtonAndNoSpinner;
            }
          },
          error => {
              this.messageService.add({severity: 'error', summary: 'Une erreur est survenue', detail: error });
              this.showButtonAndNoSpinner = !this.showButtonAndNoSpinner;
          });
    }
    else {
        this.messageService.add({severity: 'error', summary: 'Erreur rencontré dans le formulaire', detail: 'Verifiez vos entrées' });
        this.showButtonAndNoSpinner = !this.showButtonAndNoSpinner;
    }
  }
